...
int titleFinalPosition = RES_WIDTH/2 - (int)(380 * RATIO);
int titlePosition = RES_WIDTH;

int titleAnimationSpeed = int(8*RATIO);
...
void Menu::Update(Uint32 gameTime){
	...
	if(this->menuState == MENU_CREDITS){
		if(this->credits.size() <= 1){
			std::ifstream myfile ("Resources/credits.txt");
			if (myfile.is_open()){
				std::string line;
...
void Menu::Draw(SDL_Surface *viewport){
	...
	if(this->menuState == MENU_MAIN){
		for(unsigned int i = 0; i < this->menuItems.size(); i ++){
            SDL_Rect DestR; DestR.x = RES_WIDTH/3; DestR.y = int(RES_HEIGHT/2 + i*30*RATIO);
		...
		for(unsigned int i = 0; i < this->optionItems.size(); i++){
			...
			if(i == 0){
                ...
                for(j = 0; j < this->settings->GetMusicVolume(); j += 6){
                    DestR.x = (int)(RES_WIDTH/2 + j*2*RATIO); DestR.y = (int)(RES_HEIGHT/2 + i*spacing); DestR.h = (int)(30 * RATIO); DestR.w = (int)(10 * RATIO);
                    SDL_FillRect(viewport, &DestR, SDL_MapRGB(viewport->format, YELLOW.r, YELLOW.g, YELLOW.b));
				}
                for(j = j; j < 128; j += 6){
                    DestR.x = (int)(RES_WIDTH/2 + j*2*RATIO); DestR.y = (int)(RES_HEIGHT/2 + i*spacing);
                ...
                DestR.x = (int)(RES_WIDTH/2 + 100*RATIO); DestR.y = (int)(RES_HEIGHT/2 + i*spacing);
		...
		for(unsigned int i = 0; i < this->credits.size(); i++){
            SDL_Rect DestR; DestR.x = (RES_WIDTH /2) - (this->credits.at(i)->w/2); DestR.y = (int)(creditsPosition + i*30*RATIO);
